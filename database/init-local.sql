CREATE DATABASE IF NOT EXISTS htn26db;

CREATE USER IF NOT EXISTS 'htn26user'@'localhost' IDENTIFIED BY 'supersecurepassword';

GRANT ALL PRIVILEGES ON htn26db.* TO 'htn26user'@'localhost';
FLUSH PRIVILEGES;
...
ALTER TABLE accounts
  ADD COLUMN IF NOT EXISTS branch VARCHAR(255) NOT NULL DEFAULT 'Main Branch' AFTER account_name;

ALTER TABLE users
  ADD COLUMN IF NOT EXISTS avatar_url VARCHAR(500) AFTER email,
  ADD COLUMN IF NOT EXISTS firebase_uid VARCHAR(128) UNIQUE AFTER avatar_url,
  ADD COLUMN IF NOT EXISTS totp_secret VARCHAR(64) AFTER firebase_uid,
  ADD COLUMN IF NOT EXISTS totp_enabled TINYINT(1) NOT NULL DEFAULT 0 AFTER totp_secret;

ALTER TABLE transactions
  ADD COLUMN IF NOT EXISTS purpose VARCHAR(255) AFTER description,
  ADD COLUMN IF NOT EXISTS category VARCHAR(100) NOT NULL DEFAULT 'Transfers' AFTER purpose;

ALTER TABLE savings_goals
  ADD COLUMN IF NOT EXISTS daily_saving_amount DECIMAL(14, 2) NOT NULL DEFAULT 0 AFTER saved_amount;
