...
export default function PayBillsPage() {
  ...
  async function handlePayNow() {
    ...
    if (!response.ok) {
      setFailReason(payload.message || 'Payment failed.')
      setToast({
        type: 'error',
    ...
    setConfirmationNumber(String(payload.payment?.id || ''))
    setAvailableBalance(Number(payload.availableBalance))
    setToast({
      type: 'success',
      message: `Payment completed. Available balance: ${formatMoney(
        Number(payload.availableBalance)
      )}`
    })
    setScreen('success')
  }
